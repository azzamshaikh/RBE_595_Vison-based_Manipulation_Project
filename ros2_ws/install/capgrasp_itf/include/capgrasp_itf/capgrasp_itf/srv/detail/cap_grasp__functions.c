// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from capgrasp_itf:srv/CAPGrasp.idl
// generated code does not contain a copyright notice
#include "capgrasp_itf/srv/detail/cap_grasp__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
capgrasp_itf__srv__CAPGrasp_Request__init(capgrasp_itf__srv__CAPGrasp_Request * msg)
{
  if (!msg) {
    return false;
  }
  // start
  return true;
}

void
capgrasp_itf__srv__CAPGrasp_Request__fini(capgrasp_itf__srv__CAPGrasp_Request * msg)
{
  if (!msg) {
    return;
  }
  // start
}

bool
capgrasp_itf__srv__CAPGrasp_Request__are_equal(const capgrasp_itf__srv__CAPGrasp_Request * lhs, const capgrasp_itf__srv__CAPGrasp_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // start
  if (lhs->start != rhs->start) {
    return false;
  }
  return true;
}

bool
capgrasp_itf__srv__CAPGrasp_Request__copy(
  const capgrasp_itf__srv__CAPGrasp_Request * input,
  capgrasp_itf__srv__CAPGrasp_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // start
  output->start = input->start;
  return true;
}

capgrasp_itf__srv__CAPGrasp_Request *
capgrasp_itf__srv__CAPGrasp_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Request * msg = (capgrasp_itf__srv__CAPGrasp_Request *)allocator.allocate(sizeof(capgrasp_itf__srv__CAPGrasp_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(capgrasp_itf__srv__CAPGrasp_Request));
  bool success = capgrasp_itf__srv__CAPGrasp_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
capgrasp_itf__srv__CAPGrasp_Request__destroy(capgrasp_itf__srv__CAPGrasp_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    capgrasp_itf__srv__CAPGrasp_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
capgrasp_itf__srv__CAPGrasp_Request__Sequence__init(capgrasp_itf__srv__CAPGrasp_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Request * data = NULL;

  if (size) {
    data = (capgrasp_itf__srv__CAPGrasp_Request *)allocator.zero_allocate(size, sizeof(capgrasp_itf__srv__CAPGrasp_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = capgrasp_itf__srv__CAPGrasp_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        capgrasp_itf__srv__CAPGrasp_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
capgrasp_itf__srv__CAPGrasp_Request__Sequence__fini(capgrasp_itf__srv__CAPGrasp_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      capgrasp_itf__srv__CAPGrasp_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

capgrasp_itf__srv__CAPGrasp_Request__Sequence *
capgrasp_itf__srv__CAPGrasp_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Request__Sequence * array = (capgrasp_itf__srv__CAPGrasp_Request__Sequence *)allocator.allocate(sizeof(capgrasp_itf__srv__CAPGrasp_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = capgrasp_itf__srv__CAPGrasp_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
capgrasp_itf__srv__CAPGrasp_Request__Sequence__destroy(capgrasp_itf__srv__CAPGrasp_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    capgrasp_itf__srv__CAPGrasp_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
capgrasp_itf__srv__CAPGrasp_Request__Sequence__are_equal(const capgrasp_itf__srv__CAPGrasp_Request__Sequence * lhs, const capgrasp_itf__srv__CAPGrasp_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!capgrasp_itf__srv__CAPGrasp_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
capgrasp_itf__srv__CAPGrasp_Request__Sequence__copy(
  const capgrasp_itf__srv__CAPGrasp_Request__Sequence * input,
  capgrasp_itf__srv__CAPGrasp_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(capgrasp_itf__srv__CAPGrasp_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    capgrasp_itf__srv__CAPGrasp_Request * data =
      (capgrasp_itf__srv__CAPGrasp_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!capgrasp_itf__srv__CAPGrasp_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          capgrasp_itf__srv__CAPGrasp_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!capgrasp_itf__srv__CAPGrasp_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `xyz`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
capgrasp_itf__srv__CAPGrasp_Response__init(capgrasp_itf__srv__CAPGrasp_Response * msg)
{
  if (!msg) {
    return false;
  }
  // xyz
  if (!rosidl_runtime_c__double__Sequence__init(&msg->xyz, 0)) {
    capgrasp_itf__srv__CAPGrasp_Response__fini(msg);
    return false;
  }
  return true;
}

void
capgrasp_itf__srv__CAPGrasp_Response__fini(capgrasp_itf__srv__CAPGrasp_Response * msg)
{
  if (!msg) {
    return;
  }
  // xyz
  rosidl_runtime_c__double__Sequence__fini(&msg->xyz);
}

bool
capgrasp_itf__srv__CAPGrasp_Response__are_equal(const capgrasp_itf__srv__CAPGrasp_Response * lhs, const capgrasp_itf__srv__CAPGrasp_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // xyz
  if (!rosidl_runtime_c__double__Sequence__are_equal(
      &(lhs->xyz), &(rhs->xyz)))
  {
    return false;
  }
  return true;
}

bool
capgrasp_itf__srv__CAPGrasp_Response__copy(
  const capgrasp_itf__srv__CAPGrasp_Response * input,
  capgrasp_itf__srv__CAPGrasp_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // xyz
  if (!rosidl_runtime_c__double__Sequence__copy(
      &(input->xyz), &(output->xyz)))
  {
    return false;
  }
  return true;
}

capgrasp_itf__srv__CAPGrasp_Response *
capgrasp_itf__srv__CAPGrasp_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Response * msg = (capgrasp_itf__srv__CAPGrasp_Response *)allocator.allocate(sizeof(capgrasp_itf__srv__CAPGrasp_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(capgrasp_itf__srv__CAPGrasp_Response));
  bool success = capgrasp_itf__srv__CAPGrasp_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
capgrasp_itf__srv__CAPGrasp_Response__destroy(capgrasp_itf__srv__CAPGrasp_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    capgrasp_itf__srv__CAPGrasp_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
capgrasp_itf__srv__CAPGrasp_Response__Sequence__init(capgrasp_itf__srv__CAPGrasp_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Response * data = NULL;

  if (size) {
    data = (capgrasp_itf__srv__CAPGrasp_Response *)allocator.zero_allocate(size, sizeof(capgrasp_itf__srv__CAPGrasp_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = capgrasp_itf__srv__CAPGrasp_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        capgrasp_itf__srv__CAPGrasp_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
capgrasp_itf__srv__CAPGrasp_Response__Sequence__fini(capgrasp_itf__srv__CAPGrasp_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      capgrasp_itf__srv__CAPGrasp_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

capgrasp_itf__srv__CAPGrasp_Response__Sequence *
capgrasp_itf__srv__CAPGrasp_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  capgrasp_itf__srv__CAPGrasp_Response__Sequence * array = (capgrasp_itf__srv__CAPGrasp_Response__Sequence *)allocator.allocate(sizeof(capgrasp_itf__srv__CAPGrasp_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = capgrasp_itf__srv__CAPGrasp_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
capgrasp_itf__srv__CAPGrasp_Response__Sequence__destroy(capgrasp_itf__srv__CAPGrasp_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    capgrasp_itf__srv__CAPGrasp_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
capgrasp_itf__srv__CAPGrasp_Response__Sequence__are_equal(const capgrasp_itf__srv__CAPGrasp_Response__Sequence * lhs, const capgrasp_itf__srv__CAPGrasp_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!capgrasp_itf__srv__CAPGrasp_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
capgrasp_itf__srv__CAPGrasp_Response__Sequence__copy(
  const capgrasp_itf__srv__CAPGrasp_Response__Sequence * input,
  capgrasp_itf__srv__CAPGrasp_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(capgrasp_itf__srv__CAPGrasp_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    capgrasp_itf__srv__CAPGrasp_Response * data =
      (capgrasp_itf__srv__CAPGrasp_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!capgrasp_itf__srv__CAPGrasp_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          capgrasp_itf__srv__CAPGrasp_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!capgrasp_itf__srv__CAPGrasp_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
